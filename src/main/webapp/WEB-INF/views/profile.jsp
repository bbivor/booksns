	...
	<script>
		...
		*/
		...
		$(function() {
			...
			var followList = $('#followList').val();
			
			var json = JSON.parse(followList);
			
			$.each(json, function(inx, item) {
				if (item == userId) {
					$(this).html('Unfollow');
					return;
				}
			...
			$(".ui.right.floated.brown.basic.toggle.button").click(function() {
				if ($(this).html() == 'Follow') {
					$(this).toggleClass('active');
					$(this).html('Unfollow');
					location.href="follow?followid=" + followId;
				}
							...
							<c:if test="${sessionScope.userId != user.userId }">
								...
								<c:choose>
									...
									<c:when test="${follow != '[]' }">
										...
										<c:choose>
											<c:when test="${id.followId == user.userId }">
												<c:if test="${not flag }">
							...
							<div class="item">
								...
								<div class="ui horizontal list">
									<c:if test="${user.saveFile == null }">
										<div class="item">
											<img class="profileImg" src="./resources/images/defaultAvatar.png">
											<h2>${user.userId }</h2>
										</div>
										<div class="item">
											<textarea id="introduce" name="introduce" readonly="readonly" cols="50" placeholder="${user.introduce }"></textarea>
										</div>
									</c:if>
								</div>
								...
								<div class="ui horizontal list">
									<c:if test="${user.saveFile != null }">
										<div class="item">
											<img class="profileImg" src="./resources/images/FileRepo/${user.saveFile }">
											<h2>${user.userId }</h2>
										</div>
										<div class="item">
											<textarea id="introduce" name="introduce" readonly="readonly" cols="50" placeholder="${user.introduce }"></textarea>
